    ...
    with io.open(full_name, "rt", encoding="utf-8", errors="strict") as f:
        ...
        for li, line in enumerate(lines):
            ...
            if len(cols) == 0:
                if tokens:
                    ...
                    for i in range(length): 
                        if corefs[i-j]["end"] is None:
                            #recording labels that do not end in this utterance
                            extra_corefs.append(corefs[i]["label"]) 
                            del corefs[i]
                            j+=1
                    if debug:
            ...
            # Inside utterance
                ...
                assert int(cols[2]) == index, "Index error index "+str(index)+" on " + line
                if speaker:
                ...
                if cols[-1] != "-":
                    ...
                    for tok in coref_expr:
                        ...
                        if match.group(1) == "(":
                            if debug:
                                print("New coref", num)
                            corefs.append({"label": num, "start": index-1, "end": None})
                        if match.group(3) == ")":
                            if num in extra_corefs: #ignoring labels started in other utterance
                                extra_corefs.remove(num)
                            else:
                                j = None
        ...
        with io.open(key_file, "w", encoding="utf-8") as kf:
            ...
            for dirpath, _, filenames in os.walk(data_path):
                ...
                file_list = [
                    ...
                    for f in filenames
                    if f.endswith(".v4_auto_conll") or f.endswith(".v4_gold_conll") or f.endswith(".conll")
                ]
                cleaned_file_list = []
                for f in file_list:
                    fn = f.split(".")
