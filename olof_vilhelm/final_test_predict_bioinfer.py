...
if __name__ == '__main__':
    ...
    relations = list()
    output = {}
    filename = 'corpus/BioInfer_corpus_1.2.0b.binarised.xml'
    abstracts = BioInfer(filename).sentences()

    for s_id in abstracts:
        abstract = abstracts[s_id]
        pred_rels_cnt = 0
        actual_rel_cnt += len(abstract['relations'])
        dcnt += 1
        text = abstract["text"]
        doc = nlp(text)
        print("SENTENCE ID:", s_id, "FROM", filename)
        print("TEXT:")
        print(text, "\n")

        print("ACTUAL RELATIONS:")
        for relation in abstract['relations']:
            from_entity = relation['source']['text']
            to_entity = relation['target']['text']
            relation_type = relation['relation']['text']
            print(from_entity, "-", relation_type, "-", to_entity)
        print("\nPREDICTED RELATIONS")
        ...
        for chunk in doc.noun_chunks:
                ...
                for chunk2 in doc.noun_chunks:
                    if chunk2.root.dep_ == 'dobj' and chunk2.root.head == keyword:
                        ...
                        if doc[keyword.i-1].dep_ != 'neg':
                            ...
                            src = Source(doc.text, "id=" + abstract["id"])
            ...
            j += 1
        if pred_rels_cnt == 0:
            print("No predicted relations.\n")
        else:
            print()
            print(pred_rels_cnt, "predicted relations found.\n")
    ...
    print("predicted nr of relations: ", icnt)
    print("actual nr of relations: ", actual_rel_cnt)
    # print(output)
    """
    for r in output:
